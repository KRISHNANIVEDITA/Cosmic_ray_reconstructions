def energy_reco(Nc,theta):
    print("entering ENERGY REGIME")
    #print("The cvalue of shower",Ne_fit)
    C_shower = Nc
    nkg_results = pd.read_csv('/users/PAS1968/knivedita/data_analysis/nkg_fits.csv')
...
# Interpolate C at this point
    ...
    energies = np.linspace(13.0, 20.0, 48) 
    ...
    C_13 = slope * np.array(13.0) + intercept
    if log_C_shower < C_13:
...
def fit_arrival_direction(x, y, dt, length):
    ...
    fit_plane = TF2("fit_plane", func_plane, -100, 100, -100, 100, 3)
...
T1P5 = uproot.open("/users/PAS1968/knivedita/saturation_root/2s.root")
tree_t1p5 = T1P5["tree"]
...
for root_file in root_files:
    with uproot.open(root_file) as f:
        if "tree" in f:
            chain.Add(root_file)
#pdf_filename = "/users/PAS1968/knivedita/data_analysis/T1P5_arrival_core_1.1.pdf"
#with PdfPages(pdf_filename) as pdf:
lowcount = 0
highcount=0
with open('/users/PAS1968/knivedita/data_analysis/2s_1.1.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    ...
    counter=0
    ### event 71 , 100
    for sec, nsec in zip(utc_secs10[:500], utc_nsecs10[:500]):
    #for sec, nsec in zip([1719956221],[815478338]):
        try:
            ...
            if randomEntry >= 0:
                ...
                #print('printing checkkkk',event_new[check])
                ...
                Ne_fit,rM_fit,s_fit,real_x, real_y, realxerr, realyerr = helper.fit_NKG(theta_rad, phi_rad, event_new[check],100)
                print('theta is',theta)
                Nc=Ne_fit
                theta_temp=theta
                if theta_temp < 20:
                    theta_temp = 20
                if theta_temp >= 20 and theta_temp <= 50:
                    #lowcount,highcount = energy_reconstruction(Nc,theta,lowcount,highcount)
                    reco_energy_log10 = energy_reco(Nc,theta_temp)
                else : 
                    reco_energy_log10 = 'None'
                #writer.writerow([counter,sec, nsec, theta, phi, rms_error, real_x, real_y, realxerr, realyerr,reco_energy_log10])
                xcores=[]
                ycores = []
                xerror = []
                yerror =[]
                ...
                adc_count = list(range(3500, 12001, 1000))  # [3500, 4000, ..., 10000]
                event_demo = event_new.copy() 
                adclist = list(event_new['adc_counts'][check])
                energylist = list(event_new['energy_deposits(MeV)'][check])
                print('main',adclist)
                adc_check = np.array([1 if x>3200 else 0 for x in adclist])
                indices = np.where(adc_check > 0)[0]  
                print(len(indices))
                adc_counts = [adc_count] * len(indices)  # Repeat for each index
                # Iterate over each adc value in adc_count_any
                #real_x,real_y,realxerr,realyerr = fit_NKG(theta_rad,phi_rad,event)

                for combo in product(*adc_counts):
                    print(combo)
                    adcnewlist = adclist.copy()
                    energynewlist = energylist.copy()
                    for ind in range(len(combo)):
                        adcnewlist[indices[ind]] = combo[ind]
                        energynewlist[indices[ind]] = (adcnewlist[indices[ind]]/adclist[indices[ind]])*energylist[indices[ind]]
                    event_demo['adc_counts'] = adcnewlist
                    event_demo['energy_deposits(MeV)'] = energynewlist
                    ne,rm,sfit,x_core,y_core,errx,erry = helper.fit_NKG(theta_rad,phi_rad,event_demo[check],combo[ind])
                    #print(adcnewlist)
                    #3print(energynewlist)
                    xcores.append(x_core)
                    ycores.append(y_core)
                    xerror.append(errx)
                    yerror.append(erry)
                    ne_vals.append(ne)
                    rm_vals.append(rm)
                ...
                fit_vals = []
                for nn,rr,ss,xc,yc in zip(ne_vals,rm_vals,s_vals,xcores,ycores):
                    nkg_single = (nn/np.power(rr,2))*(TMath.Gamma(4.5-ss))/(2*np.pi*(TMath.Gamma(ss))*(TMath.Gamma(4.5-2*ss)))*np.power(np.sqrt(np.power(xc-X,2)+np.power(yc-Y,2))/rr,ss-2)*np.power(1.0+np.sqrt(np.power(xc-X,2)+np.power(yc-Y,2))/rr,ss-4.5)
                    fit_vals.append(nkg_single)            

                print(len(fit_vals))
                chi2=[]
                adc_count = list(range(3500, 12001, 1000)) 
                event_demo = event_new.copy() 
                adclist = list(event_new['adc_counts'][check])
                energylist = list(event_new['energy_deposits(MeV)'][check])
                adc_check = np.array([1 if x>3200 else 0 for x in adclist])
                indices = np.where(adc_check > 0)[0]  
                adc_counts = [adc_count] * len(indices) 
                combos = list(product(*adc_counts))
                for combo in product(*adc_counts):
                    adcnewlist = adclist.copy()
                    energynewlist = energylist.copy()
                    for ind in range(len(combo)):
                        adcnewlist[indices[ind]] = combo[ind]
                        energynewlist[indices[ind]] = (adcnewlist[indices[ind]]/adclist[indices[ind]])*energylist[indices[ind]]
                    diff_energy = np.sqrt(np.mean((np.array(energynewlist) - np.array(fit_vals)) ** 2))
                    chi2.append(diff_energy)
                    
                min_index = np.argmin(chi2)
                best_combo = combos[min_index]
                new_xcore = xcores[min_index]
                new_ycore = ycores[min_index]
                new_Nc = ne_vals[min_index]
                if theta_temp >= 20 and theta_temp <= 50:
                    #lowcount,highcount = energy_reconstruction(Nc,theta,lowcount,highcount)
                    new_reco_energy_log10 = energy_reco(new_Nc,theta_temp)
                else : 
                    new_reco_energy_log10 = 'None'
                writer.writerow([counter,sec, nsec, theta, phi, rms_error, real_x, real_y, realxerr, realyerr,new_xcore,new_ycore,reco_energy_log10,new_reco_energy_log10])
        except:
            continue
